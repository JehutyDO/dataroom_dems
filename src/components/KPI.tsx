        ...
        <CardContent className="p-4 sm:p-6 h-full flex items-center">
          <div className="flex items-center justify-between gap-4 w-full">
            <div className="flex-1 min-w-0">
              <p className="text-xs md:text-sm text-foreground/70 mb-1 sm:mb-2 min-h-[1.25rem]">{title}</p>
              <p className="text-2xl md:text-3xl font-semibold text-foreground truncate min-h-[2rem] sm:min-h-[2.5rem]">{value}</p>
              {trend && (
                <p className="text-xs md:text-sm text-blue-500 mt-1 sm:mt-2 min-h-[1.25rem]">{trend}</p>
              )}
            </div>
